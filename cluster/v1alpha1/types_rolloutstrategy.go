...
type RolloutConfig struct {
	...
	// The default value is 0 meaning the workload applier proceeds immediately after a successful state is reached.
	...
	// ProgressDeadline defines how long workload applier controller will wait for the workload to reach a successful state in the cluster.
	// ProgressDeadline default value is "None", meaning the workload applier will wait for a successful state indefinitely.
	...
	// MaxFailures is a percentage or number of clusters in the current rollout that can fail before proceeding to the
	// next rollout.
	// MaxFailures is only considered for rollout types Progressive and ProgressivePerGroup. For Progressive, this is
	// considered over the total number of clusters. For ProgressivePerGroup, this is considered according to the size of
	// the current group.
	...
	// Timeout defines how long the workload applier controller will wait until the workload reaches a successful state in
	// the cluster.
	// Timeout default value is None meaning the workload applier will not proceed apply workload to other clusters if did
	// not reach the successful state.
...
type RolloutProgressive struct {
	...
	// MaxConcurrency is the max number of clusters to deploy workload concurrently. The default value for MaxConcurrency
	// is determined from the clustersPerDecisionGroup defined in the placement->DecisionStrategy.
	// +kubebuilder:validation:Pattern="^((100|[0-9]{1,2})%|[0-9]+)$"
	// +kubebuilder:validation:XIntOrString
	// +optional
