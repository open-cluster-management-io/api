...
var map_AddOnPlacementScoreStatus = map[string]string{
	...
	"scores":     "Scores contains a list of score name and value of this managed cluster.",
...
var map_PrioritizerAddOnScore = map[string]string{
...
var map_PrioritizerScoreCoordinate = map[string]string{
	...
	"type":    "Type defines the type of the prioritizer. Type is either \"BuildIn\", \"AddOn\" or \"\", where \"\" is \"BuildIn\" by default. When the type is \"BuildIn\", need to specify a buildin prioritizer name in BuildIn. When the type is \"AddOn\", need to configure the score source in AddOn.",
	"buildIn": "BuildIn defines the name of a buildin prioritizer. Below are the valid buildin prioritizer names. 1) Balance: balance the decisions among the clusters. 2) Steady: ensure the existing decision is stabilized. 3) ResourceAllocatableCPU & ResourceAllocatableMemory: sort clusters based on the allocatable.",
	"addOn":   "When type is \"AddOn\", AddOn defines the resource name and score name.",
}

func (PrioritizerScoreCoordinate) SwaggerDoc() map[string]string {
	return map_PrioritizerScoreCoordinate
}
