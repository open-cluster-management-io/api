...
var map_CreatedManifestResourceMeta = map[string]string{
	"":          "CreatedManifestResourceMeta represents the gvr, name and namespace of a resource Since these resources have been created, they must have valid group, version, resource, namespace, and name",
...
func (CreatedManifestResourceMeta) SwaggerDoc() map[string]string {
	return map_CreatedManifestResourceMeta
}
...
var map_ManifestWorkStatus = map[string]string{
	...
	"createdResources": "CreatedResources represents every resource that has been created for this ManifestWork and has not yet been deleted. This means that only resources with valid GroupVersionResource, namespace, and name are suitable. After a finalizer or controller deletes an item from this slice, it may be removed. The deleted resource may still be present until the finalizers for that resource are finished. However, the resource will not be undeleted, so it can be removed from this list and eventual consistency is preserved.",
}
