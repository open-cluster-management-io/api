...
type ManagedClusterAddOnLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.ManagedClusterAddOn, err error)
...
// List lists all ManagedClusterAddOns in the indexer.
func (s *managedClusterAddOnLister) List(selector labels.Selector) (ret []*v1beta1.ManagedClusterAddOn, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.ManagedClusterAddOn))
	})
	return ret, err
}
...
type ManagedClusterAddOnNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.ManagedClusterAddOn, error)
...
// List lists all ManagedClusterAddOns in the indexer for a given namespace.
func (s managedClusterAddOnNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.ManagedClusterAddOn, err error) {
...
// Get retrieves the ManagedClusterAddOn from the indexer for a given namespace and name.
func (s managedClusterAddOnNamespaceLister) Get(name string) (*v1beta1.ManagedClusterAddOn, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("managedclusteraddon"), name)
	}
	return obj.(*v1beta1.ManagedClusterAddOn), nil
}
